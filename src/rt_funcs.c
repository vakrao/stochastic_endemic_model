...
float mod_rt_calc(double* S,double* I,double* R,double* V, double* N,double** M,double* mu, double* m,double q1,struct ParameterSet p){
    ...
    for(int i = 0; i < p.AGES*2; i++){
       ...
       for(int k = 0; k < p.AGES;k++){
        ...
        for(int j = 0; j < p.AGES*2;j++){
            double A = S[row_counter]*(m_val/totalN); 
            double B = m_val*(S[row_counter]/totalN)*p.sigma_d1; 
            double C = m_val*(V[row_counter]/totalN)*(1-p.sigma_i1); 
            double D = m_val*(V[row_counter]/totalN)*p.sigma_q1*(1-p.sigma_i1); 
             if((i % 2 == 0) && (j % 2 == 0)){
                 gsl_matrix_set(G,i,j,A);
             } 
             if((i % 2 == 0) && (j % 2 == 1)){
...
//float mod_q_calc(double* S,double* I,double* R,double* V, double* N,double** M,double* mu, double* m,int R0,struct ParameterSet p){
//    gsl_matrix *FL = gsl_matrix_alloc(p.AGES*2,p.AGES*2);
//    gsl_matrix *G = gsl_matrix_alloc(p.AGES*2,p.AGES*2);
//    gsl_permutation *inv_perm = gsl_permutation_alloc(p.AGES*2);
//    gsl_matrix *K = gsl_matrix_alloc(p.AGES*2,p.AGES*2);
